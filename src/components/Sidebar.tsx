  ...
  const sidebarContent = (
    <div className="flex flex-col h-full bg-white border-r border-[#d2d2d2] w-60 select-none">
      ...
      <div className="flex flex-col gap-1.5 p-4 border-b border-[#f1f1f1]">
        ...
        <p className="text-[9px] font-bold text-stone-400 uppercase tracking-widest pl-0.5 mt-1">Enterprise Console</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto py-4 px-3 space-y-4 scrollbar-thin">
        ...
        <div className="space-y-1">
          <button onClick={() => handleTabClick('dashboard')} className={navClass('dashboard')}>
        ...
        <div className="space-y-1">
          <div className="flex items-center justify-between px-3 mb-1">
            <p className="text-[10px] font-bold text-stone-400 uppercase tracking-wider">Catalog</p>
            <button
            ...
            >
              {catalogExpanded ? <ChevronDown className="h-3 w-3" /> : <ChevronRight className="h-3 w-3" />}
            </button>
          </div>

          {catalogExpanded && (
            <div className="space-y-1">
              <button onClick={() => handleTabClick('products')} className={subNavClass('products')}>
              ...
              <button onClick={() => handleTabClick('categories')} className={subNavClass('categories')}>
                <span className="flex items-center gap-2">
                  <Layers className="h-3.5 w-3.5 text-stone-400" />
                  Categories
                </span>
              </button>
              <button onClick={() => handleTabClick('hampers')} className={subNavClass('hampers')}>
                <span className="flex items-center gap-2">
        ...
        <div className="space-y-1">
          <div className="flex items-center justify-between px-3 mb-1">
            <p className="text-[10px] font-bold text-stone-400 uppercase tracking-wider">Orders & Logistics</p>
            <button
              onClick={() => setOrdersExpanded(!ordersExpanded)}
              className="text-stone-400 hover:text-stone-600 p-0.5 rounded-sm transition-colors"
            >
              {ordersExpanded ? <ChevronDown className="h-3 w-3" /> : <ChevronRight className="h-3 w-3" />}
            </button>
          </div>

          {ordersExpanded && (
            <div className="space-y-1">
              <button onClick={() => handleTabClick('orders')} className={subNavClass('orders')}>
                <span className="flex items-center gap-2">
                  <PackageCheck className="h-3.5 w-3.5 text-stone-400" />
                  All Orders
                </span>
              </button>
              <button onClick={() => handleTabClick('shipping')} className={subNavClass('shipping')}>
                <span className="flex items-center gap-2">
        ...
        {/* Customer Operations */}
        <div className="space-y-1">
          <p className="px-3 text-[10px] font-bold text-stone-400 uppercase tracking-wider mb-1">Customers</p>

          <button onClick={() => handleTabClick('customers')} className={navClass('customers')}>
            <span className="flex items-center gap-3">
        ...
        {/* CMS & Content */}
        <div className="space-y-1">
          <p className="px-3 text-[10px] font-bold text-stone-400 uppercase tracking-wider mb-1">Marketing & CMS</p>

          <button onClick={() => handleTabClick('coupons')} className={navClass('coupons')}>
            <span className="flex items-center gap-3">
          ...
          <button onClick={() => handleTabClick('cms')} className={navClass('cms')}>
            <span className="flex items-center gap-3">
        ...
        {/* Compliance & Partners */}
        <div className="space-y-1">
          <p className="px-3 text-[10px] font-bold text-stone-400 uppercase tracking-wider mb-1">Partners</p>

          <button onClick={() => handleTabClick('suppliers')} className={navClass('suppliers')}>
            <span className="flex items-center gap-3">
          ...
          <button onClick={() => handleTabClick('certificates')} className={navClass('certificates')}>
            <span className="flex items-center gap-3">
        ...
        <div className="space-y-1 pt-2 border-t border-[#f1f1f1]">
          <p className="px-3 text-[10px] font-bold text-stone-400 uppercase tracking-wider mb-1">Analysis & Config</p>
          
          <button onClick={() => handleTabClick('analytics')} className={navClass('analytics')}>
            <span className="flex items-center gap-3">
          ...
          <button onClick={() => handleTabClick('settings')} className={navClass('settings')}>
            <span className="flex items-center gap-3">
...
function GiftHamperIcon(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg
      ...
      {...props}
    >
      <path strokeLinecap="round" strokeLinejoin="round" d="M12 8v13m0-13A3.5 3.5 0 1 0 8.5 4.5c.348 0 .686.046 1.01.13M12 8a3.5 3.5 0 1 1 3.5-3.5c-.347 0-.685.046-1.01.13M12 8V4.63M3 12h18M4 12v7a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-7M3 9.5a2.5 2.5 0 0 1 2.5-2.5h13A2.5 2.5 0 0 1 21 9.5v2.5H3V9.5Z" />
    </svg>
  );
}
